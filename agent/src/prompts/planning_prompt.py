...
- **CRITICAL: You MUST select objects ONLY from <object_text>. Never invent or assume object names that are not explicitly listed.**
- **If an object mentioned in the subgoal does not exist in <object_text>, find the closest matching object or skip the task.**
...
- Each task step must include:
  - the skill name,
  - the target object (must be an exact match from object_text).
